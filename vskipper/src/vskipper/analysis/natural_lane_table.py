    ...
    for ds, labels in LABELS.items():
        for lbl, rate in labels:
            ...
            for arm, r in (("upstream", u), ("vskipper", v)):
                if a.compact:
                    rows.append(f"{NAMES[ds] if arm == 'upstream' else ''} & {rate if arm == 'upstream' else ''} & {STACK[arm]} & {r['mean_out_tokens']:.0f} & {r['cap_hits']} & {r['mean_e2e_s']:.1f} & {r['duration_s']:.0f} \\\\")
                else:
            ...
            if w is not None and ("alwaysroute", ds, lbl) in lm:
                if a.compact:
                    rows.append(f" & & FlexiDepth always-route & {w['mean_out_tokens']:.0f} & {w['cap_hits']} & {w['mean_e2e_s']:.1f} & {w['duration_s']:.0f} \\\\")
                else:
