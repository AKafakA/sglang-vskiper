...
EMPIRICALLY: ``vskipper/src/vskipper/experiments/gates/route_digest_compare.py`` (route counters + output
text sha256) plus a bit-exact ``route_weight``/``route_mask`` capture through
``vskipper/src/vskipper/experiments/compare_fd_parity_traces.py``.
...
class FDRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # vskipper/tests/test_fd_rmsnorm_fused.py against `reference_forward`; any
...
def _route_decide_stats(device: torch.device) -> torch.Tensor:
    from vskipper.runtime.env import _ROUTE_DECIDE_STATS
