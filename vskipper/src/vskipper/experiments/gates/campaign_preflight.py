...
def main() -> int:
    ...
    ok &= check("served fork tree", (a.tree / "vskipper/src/vskipper/runtime/design.py").is_file(), str(a.tree))
    ...
    up_vpipe = a.upstream / "vskipper/src/vskipper/runtime"
    ...
    manifest = a.tree / "vskipper/configs/deploy/upstream_baseline.json"
    ...
    if a.serving_pythonpath is not None:
        want = ([a.upstream / "python"] if a.serving_is_upstream else
                [a.tree / "vskipper/src", a.tree / "python"])
        got = [Path(p).resolve() for p in a.serving_pythonpath.split(os.pathsep) if p]
        same = got == [p.resolve() for p in want]
        ok &= check("serving PYTHONPATH is the verified tree", same,
                    f"{got} vs {want}" if not same else str(got))

    arm_file = a.tree / "vskipper/configs/deploy/active_arm"
    ok &= check("vskipper/configs/deploy/active_arm present", arm_file.is_file(),
