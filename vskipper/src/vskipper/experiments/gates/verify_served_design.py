...
def load_design(tree: Path):
    ...
    """
    path = tree / "vskipper" / "src" / "vskipper" / "runtime" / "design.py"
    if not path.is_file():
...
def intended(tree: Path, arm: str) -> dict[str, Any]:
    ...
    try:
        ...
        # is verifying -- an interrupted run would leave vskipper/configs/deploy/active_arm rewritten.
        from vskipper.runtime import design as _d
