...
spec = importlib.util.spec_from_file_location(
    "vpipe_design", tree / "vskipper" / "src" / "vskipper" / "runtime" / "design.py")
d = importlib.util.module_from_spec(spec); spec.loader.exec_module(d)
d.resolve_arm(arm)                                    # fails closed on an unknown arm
(tree / "vskipper/configs/deploy").mkdir(parents=True, exist_ok=True)
(tree / "vskipper/configs/deploy" / "active_arm").write_text(arm + "\n")
print(f"active arm -> {arm}")
ARMPY
[ $? -eq 0 ] || { echo "FATAL: arm $ARM is not a canonical arm"; exit 2; }
export SGLANG_VP_HOST_CONFIG="${VP_GATE_HOST_CONFIG:-$TREE/vskipper/configs/deploy/hosts/vast-a100.json}"
...
export PYTHONPATH="$TREE/vskipper/src:$TREE/python"
{
...
if ! $V "$TREE/vskipper/src/vskipper/experiments/gates/verify_served_design.py" \
...
$V "$TREE/vskipper/src/vskipper/experiments/fdpre_label_probe.py" --url "http://127.0.0.1:$PORT" \
