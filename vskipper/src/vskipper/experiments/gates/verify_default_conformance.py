...
def upstream_defaults(defaults_tree: Path, model_path: str, python: str) -> dict[str, Any]:
    env = dict(os.environ)
    env["PYTHONPATH"] = os.pathsep.join((str(defaults_tree / "vskipper/src"), str(defaults_tree / "python"))) if (defaults_tree / "vskipper/src").is_dir() else str(defaults_tree / "python")
    for key in list(env):
...
def _read_declarations(design_tree: Path) -> dict[str, Any] | None:
    env = dict(os.environ)
    env["PYTHONPATH"] = os.pathsep.join((str(design_tree / "vskipper/src"), str(design_tree / "python")))
    code = ("import json; from vskipper.runtime import design; "
