...
validate_sgbench_protocol() {
  ...
  case "$SGBENCH_EVIDENCE_CLASS" in
    ...
    performance|headline)
      die "$SGBENCH_EVIDENCE_CLASS evidence must use the manifested deployment plus vskipper/src/vskipper/experiments/run_qps_evaluation.py; the generic ladder is diagnostic-only"
      ;;
    *)
...
start_load_sampler() {
  local port="$1"
  local output_path="$2"
  echo "+ python vskipper/src/vskipper/experiments/sample_sglang_load.py --base-url http://127.0.0.1:$port --output $output_path --interval-ms $SGBENCH_LOAD_SAMPLE_INTERVAL_MS &"
  if [[ "$DRY_RUN" == "0" ]]; then
    python vskipper/src/vskipper/experiments/sample_sglang_load.py \
...
write_sgbench_manifest() {
  ...
  mkdir -p "$SGBENCH_OUT_DIR"
  {
    ...
    echo "[source-sha256]"
    sha256sum \
      "$ROOT"/vskipper/src/vskipper/runtime/*.py \
      ...
      "$ROOT"/vskipper/src/vskipper/experiments/sample_sglang_load.py \
      "$ROOT"/vskipper/src/vskipper/experiments/vast_a100_ladder.sh
    echo
    echo "[sgbench-config]"
    echo "MODEL=$MODEL"
    echo "PROFILE_FILE=$PROFILE_FILE"
...
Usage:
  ...
  DRY_RUN=1 bash vskipper/src/vskipper/experiments/vast_a100_ladder.sh
...
performance decision. Use vskipper/src/vskipper/experiments/launch_qps_server.py plus
vskipper/src/vskipper/experiments/run_qps_evaluation.py for the open-loop labeled workload and omit
