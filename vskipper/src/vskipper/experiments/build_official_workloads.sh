...
CONFIG="${CONFIG:-$ROOT/vskipper/src/vskipper/experiments/labeled_workload.example.json}"
...
# vskipper/src/vskipper/experiments/labeled_workload.example.json, which is not in this tree, and it was
...
for required in "$CONFIG" "$ROOT/vskipper/src/vskipper/experiments/build_labeled_workload.py" \
                "$ROOT/vskipper/src/vskipper/experiments/freeze_workload_suite.py"; do
  if [[ ! -e "$required" ]]; then
...
build_one() {
  ...
  local count="$2"
  shift 2
  "$PYTHON_BIN" "$ROOT/vskipper/src/vskipper/experiments/build_labeled_workload.py" \
    --workload "$workload" \
...
"$PYTHON_BIN" "$ROOT/vskipper/src/vskipper/experiments/freeze_workload_suite.py" \
