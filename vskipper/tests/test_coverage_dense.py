...
from vskipper.runtime import coverage as coverage_dense
from vskipper.runtime import attestation as vpipe_attestation
...
def test_decode_regime_kv_criterion_keys_on_resident_tokens() -> None:
    ...
    from vskipper.runtime.regime import DecodeRegimeDispatch
...
def test_native_dense_mlp_applies_the_declared_gate_arithmetic() -> None:
    ...
    from vskipper.kernels.mlp import _native_dense_mlp

    rows, hidden = 6, 8
    torch.manual_seed(0)
    hidden_states = torch.randn(rows, hidden)
    route_weights = torch.rand(rows, 1)
    run_mask = torch.tensor([[True], [False], [True], [False], [True], [False]])
    ...
    def projector(value: torch.Tensor) -> torch.Tensor:
        return value * 3.0
