...
PKG = pathlib.Path(__file__).resolve().parents[2] / "vskipper" / "src" / "vskipper"
...
def module_paths():
    return [p for p in PKG.rglob("*.py")
            if p.relative_to(PKG).parts[0] not in {"analysis", "experiments"}]
...
def scan():
    ...
    for p in sorted(module_paths()):
        ...
        if bad:
            problems[str(p.relative_to(PKG))] = bad
    return problems
...
if __name__ == "__main__":
    ...
    print(f"MODULE IMPORT SAFETY: {'FAIL' if found else 'PASS'} ({len(module_paths())} modules)")
