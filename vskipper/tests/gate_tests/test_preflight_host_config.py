...
`/nonexistent/hpc-cluster/...` paths: it had validated `hpc-cluster.json`, first alphabetically, on a box that
is not that cluster, while the campaign contract named `vskipper/configs/deploy/hosts/vast-a100.json`.
...
def test_it_checks_the_NAMED_config_not_the_alphabetically_first(tmp_path):
    """hpc-cluster sorts before vast-a100 and points at paths that do not exist here. The gate must
    report on vast-a100 -- the one the run names -- and never mention the cluster paths."""
    real = tmp_path / "staged"
    real.mkdir()
    tree = _tree(tmp_path, {
        "hpc-cluster": {"flexidepth_weights": "/nonexistent/hpc-cluster/nope/router.pt",
                 "conditional_graph_helper": "/nonexistent/hpc-cluster/nope/helper.so",
                 "moe_config_dir": "/nonexistent/hpc-cluster/nope/moe"},
    ...
    assert "/nonexistent/hpc-cluster" not in done.stdout, (
        "the gate validated the cluster's config on a host that is not that cluster:\n" + done.stdout
    )
...
def test_a_named_config_that_does_not_exist_is_REFUSED(tmp_path):
    tree = _tree(tmp_path, {"hpc-cluster": {"flexidepth_weights": "/x", "conditional_graph_helper": "/x",
                                     "moe_config_dir": "/x"}})
    done = _run(tree, tmp_path, "vskipper/configs/deploy/hosts/vast-a100.json")
    assert done.returncode != 0
