...
DRIVER = (ROOT / "vskipper/src/vskipper/experiments/run_paired_campaign.py").read_text()
RUNNER = (ROOT / "vskipper/src/vskipper/experiments/run_qps_evaluation.py").read_text()
...
def _load_driver():
    ...
    path = ROOT / "vskipper/src/vskipper/experiments/run_paired_campaign.py"
...
def test_vskipper_is_the_served_arm_and_integrated_it4_is_its_alias():
    from vskipper.runtime.design import ARMS
    assert ARMS["vskipper"] == {"skipper": "flexidepth", "phases": "both", "regime_switch": True}
    assert ARMS["integrated_it4"] is ARMS["vskipper"]
    import pathlib
    active = pathlib.Path(__file__).resolve().parents[3] / "vskipper/configs/deploy/active_arm"
