...
KERNEL = ROOT / "vskipper/src/vskipper/kernels/kernel.py"
CONFIGS = ROOT / "vskipper/src/vskipper/kernels/binary_cohort_configs"
...
def test_h100_form_factors_do_not_merge_and_have_roofline_entries():
    ...
    import json
    from vskipper.kernels.kernel import canonical_device_key
    from vskipper.runtime.roofline import derived_decode_band, ridge_rows
    ...
    table = json.loads((ROOT / "vskipper/src/vskipper/runtime/device_roofline.json").read_text())
...
def test_kv_band_rule_reproduces_the_served_a100_band_and_predicts_h100():
    from vskipper.runtime.design import SERVED_REGIME_SWITCH
...
def test_kv_band_rule_refuses_a_retuned_band():
    from vskipper.runtime.roofline import assert_kv_band_follows_rule
    try:
...
def test_every_declared_device_band_follows_the_rule():
    from vskipper.runtime.design import SERVED_DECODE_KV_BAND_BY_DEVICE, SERVED_REGIME_SWITCH
    from vskipper.runtime.roofline import derived_kv_band
