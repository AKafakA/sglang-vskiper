...
Run on a CUDA host (the kernel is Triton):  python -m pytest vskipper/tests/test_route_maps_masked.py -q
...
def _reference(run_mask, valid_rows, stats):
    from vskipper.runtime.routing import build_route_maps
...
def test_masked_route_maps_identical(rows, with_valid, seed):
    from vskipper.runtime.routing import build_route_maps_from_mask
