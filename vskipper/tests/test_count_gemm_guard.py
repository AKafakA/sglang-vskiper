...
_LAUNCH = _PREAMBLE + textwrap.dedent(
    """
    from vskipper.kernels.kernel import count_matmul_gridexit
...
def _run(code: str, *argv: str, tree: str | None = None) -> subprocess.CompletedProcess:
    env = dict(os.environ)
    if tree:
        module_root = os.path.join(tree, "vskipper", "src")
        paths = [os.path.join(tree, "python")]
        if os.path.isdir(module_root):
            paths.insert(0, module_root)
        else:
            code = code.replace("from vskipper.kernels.kernel import", "from sglang.srt.vpipe.kernel import")
...
def test_guard_layout_faults_one_past_the_end():
    ...
    if proc.returncode == 0 or "SUCCEEDED" in proc.stdout:
        # Measured on a cluster A100 node (2026-09-12): the expandable-segment layout did NOT leave the
