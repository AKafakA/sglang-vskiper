...
def test_force_run_rows_marks_the_stock_pinned_rows_of_a_mixed_batch() -> None:
    from vskipper.runtime.batch import vp_force_run_rows, vp_split_decode_forward_batch
...
def test_split_chunks_each_pin_group_to_the_ladder_top_in_order() -> None:
    from sglang.srt.layers.logits_processor import LogitsProcessorOutput
    from vskipper.runtime.batch import vp_merge_logits_outputs, vp_split_decode_forward_batch
...
def test_partition_keeps_the_first_sub_pass_logits_when_the_next_replay_overwrites_the_buffer() -> None:
    ...
    from sglang.srt.layers.logits_processor import LogitsProcessorOutput
    from vskipper.runtime.batch import vp_detach_logits_output, vp_merge_logits_outputs
...
def test_admission_pin_counters_are_not_deployment_identity() -> None:
    ...
    spec = importlib.util.spec_from_file_location(
        "qps_deployment", pathlib.Path(__file__).resolve().parents[1] / "src/vskipper/experiments/qps_deployment.py"
    )
...
def test_split_refuses_uniform_extend_and_speculative_batches() -> None:
    ...
    from vskipper.runtime.batch import vp_split_decode_forward_batch
...
def test_forward_batch_init_refuses_a_mixed_extend_batch() -> None:
    from sglang.srt.model_executor.forward_batch_info import ForwardMode
    from vskipper.runtime.regime import batch_pin_of as _bpo
...
def test_pinner_one_way_upgrade_stock_to_fd_at_band_high() -> None:
    ...
    from vskipper.runtime.regime import upgrade_pinned_rows
...
def test_monotone_lane_resolves_and_assigns_rows(monkeypatch) -> None:
    ...
    from vskipper.runtime import design
    from vskipper.runtime.common import _REGIME_SWITCH_CONFIG_CACHE, regime_switch_config
    from vskipper.runtime.regime import monotone_assign_rows
...
def test_one_switch_either_direction() -> None:
    ...
    from vskipper.runtime import design
    from vskipper.runtime.regime import downgrade_pinned_rows, upgrade_pinned_rows
