...
PKG = ROOT / "vskipper" / "src" / "vskipper"
SOURCE_PATH = str(ROOT / "vskipper/src") + ":" + str(ROOT / "python")
...
def module_names():
    ...
    for p in sorted(PKG.rglob("*.py")):
        ...
        if rel.parts[0] in {"analysis", "experiments"}:
            continue  # CLI tools have their own entrypoint checks; this gate covers serving.
        parts = [x for x in rel.parts if x != "__init__"]
        names.append(".".join(["vskipper", *parts]) if parts else "vskipper")
...
def import_one(mod, extra_env):
    ...
    env = {"PYTHONPATH": SOURCE_PATH, "PATH": "/usr/bin:/bin",
           "HOME": "/tmp"}
...
def flashinfer_available():
    ...
    r = subprocess.run(
        ...
        env={"PYTHONPATH": SOURCE_PATH, "PATH": "/usr/bin:/bin", "HOME": "/tmp"},
    )
