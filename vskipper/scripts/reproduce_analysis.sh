...
[ -f $Q8ROOT/paired_report.upstream_g1024__$Q8ARM.json ] && python3 $VP/paper_table.py $Q8ROOT/paired_report.upstream_g1024__$Q8ARM.json --knee gsm8k_q8b=14 --emit --columns main --dataset-label "Qwen3-8B" --macros ./scratch/qwen8b_macros.tex --macro-prefix vpQwenEight > generated/qwen8b_serving_rows.tex && cat ./scratch/qwen8b_macros.tex >> generated/qwen_serving_macros_v16.tex && echo "  Qwen3-8B: $(grep -c '\\\\' generated/qwen8b_serving_rows.tex) rows"

say "6e Qwen quality: block-B knee rows (qwen_quality_v16) + the 2x2 per checkpoint (A/B native evaluation; C/D = block B)"
