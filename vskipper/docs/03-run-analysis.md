...
Check out the reference commit recorded in the pack's source-provenance file:

```bash
PACK=/path/to/unpacked-pack bash vskipper/scripts/reproduce_analysis.sh \
  /path/to/new-repository-output
```

This runs `vskipper/src/vskipper/analysis/` against the same evidence and checks
the same references. `vskipper/scripts/reproduce_analysis.sh` is results-only;
