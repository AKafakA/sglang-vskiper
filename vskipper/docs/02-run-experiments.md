...
```bash
bash vskipper/scripts/run.sh experiments/make_expected_runtime.py \
  --out-dir /path/to/expectations --arm upstream_g1024 --arm vskipper

bash vskipper/scripts/run.sh experiments/gates/campaign_preflight.py \
  --tree "$PWD" --upstream /path/to/upstream --workdir /path/to/staging \
  --host-config /path/to/host.json \
  --serving-pythonpath "$PWD/vskipper/src:$PWD/python"
```
...
```bash
bash vskipper/scripts/run.sh experiments/bank_from_harvest.py \
  /path/to/upstream-harvest --out /path/to/new-banks

bash vskipper/scripts/run.sh experiments/pin_output_lengths_from_bank.py \
...
```bash
bash vskipper/scripts/run.sh experiments/qstar_int.py /path/to/ladder --emit
```
...
```bash
bash vskipper/scripts/run.sh experiments/run_paired_campaign.py \
...
```bash
bash vskipper/scripts/run.sh experiments/gates/verify_served_design.py \
...
snapshots using the matching declaration under `vskipper/configs/deploy/`.
