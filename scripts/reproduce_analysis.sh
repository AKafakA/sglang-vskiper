...
# The 2x2 lm-eval samples (arms A-D, all three datasets) ship as one checksummed tarball; the
# always-route arm's GSM8K root is also an input of the client-equivalence check in step 4.
Q=${Q:-$OUT/.q2x2}
...
Q_ALL_D=$Q/opt/vpipe/campaign/q2x2-corrected/integrated_alwaysskip/gsm8k
...
python3 "$VP/loaded_quality_table.py" "$LOADED" --shares "$OUT/loaded_shares.json" \
  --rows "$OUT/loaded_quality_rows.tex" --sweep-rows "$OUT/loaded_quality_sweep.tex" \
  --macros "$OUT/loaded_quality_macros.tex"
# Appendix E.3: the two clients of the quality table, compared on the always-route arm's GSM8K documents at
# each load point (lm-eval's archived responses vs the serving benchmark's, paired by request id).
QR=$PACK/quality-lane-raw; QD=$Q_ALL_D
python3 "$VP/client_equivalence.py" --lmeval "$QD" \
  --cells "$QR/harvest-integrated_alwaysskip-gsm8k-r8p25" "$QR/harvest-integrated_alwaysskip-gsm8k-r10p45" \
          "$QR/harvest-integrated_alwaysskip-gsm8k-r13p75" \
  --loaded "$LOADED" --suite-meta "$PACK/suites/gsm8k.qual.metadata.jsonl" \
  --macros "$OUT/client_equivalence_macros.tex" --json "$OUT/client_equivalence.json"
...
# The Qwen always-route arm's attested per-phase skip ratio (App. K)
QC=$PACK/qwen/serving/step8_qwen/rep1/gsm8k/vskipper_qwen3_4b/cells
python3 "$VP/attested_skip.py" \
  --cell "Qwen=$PACK/qwen/attest/step8c_attest/rep1/gsm8k/vskipper_qwen3_4b_alwaysroute/server_info.after.json" \
  --engagement-range "Qwen=$QC/gsm8k_eqw_r10p5_qps10p5_rep1.server_info.after.json,$QC/gsm8k_eqw_r13p3_qps13p3_rep1.server_info.after.json,$QC/gsm8k_eqw_r17p5_qps17p5_rep1.server_info.after.json" \
  --macros "$OUT/attested_skip_macros.tex"
...
# paired_dod_2x2.py APPENDS rows and macros: both files start empty, or a second run doubles every \newcommand
rm -f "$OUT/quality_rows.tex" "$OUT/quality_macros.tex"
...
if [ -d "$PAPER/generated" ]; then
  ...
  # and the other direction: a paper fragment this pack does not regenerate is named, not skipped
  not_regen=0
  for f in "$PAPER"/generated/*.tex; do
    b=$(basename "$f"); [ "$b" = bank_policy.tex ] && continue
    [ -f "$OUT/$b" ] || { echo "  NOT REGENERATED (full raw tier): $b"; not_regen=$((not_regen+1)); }
  done
  echo "  byte-identical $same | differing $diff_n | not in paper $missing | not regenerated by this pack $not_regen"
  [ "$diff_n" = 0 ] && echo "REPRODUCED: all $same regenerated fragments match $PAPER/generated"
  # Appendix M's promise, checked the other way round: every result literal typed in main.tex must be
  # backed by a generated fragment or listed, with its source, in the paper's accepted-literals file.
  # A literal in neither is a hand-typed result and fails the run.
  python3 "$VP/check_paper_numbers.py" "$PAPER/main.tex" "$OUT" --allow "$PAPER/accepted_literals.tsv"
else
