...
def cell_files(root):
    """The results .jsonl files of a harvest root. Usually one. A root that was harvested on two suites
    (the always-route BBH knee root holds a 3-shot raw cell and a .qual cell) yields one bank per suite,
    each under its own suite-named subdirectory, never a sort-order pick. A runner-refused cell (renamed
    INVALID.*) has no lengths worth pinning and is not a candidate; a root with no cell is an error."""
    c = sorted(f for f in glob.glob(os.path.join(root, "cell", "*_rep*.jsonl"))
               if "arrival" not in f and ".load." not in f and not os.path.basename(f).startswith("INVALID"))
    if not c:
        sys.exit(f"FATAL {root}: no cell results file")
    return c
...
def main():
    ...
    for root in a.roots:
        root = os.path.normpath(root)   # a trailing slash would make basename() empty and collapse every bank onto one dir
        files = cell_files(root)
        for f in files:
            write_bank(root, f, a.out, suite_dir=len(files) > 1); n_ok += 1
    print(f"wrote {n_ok} banks under {a.out}")


def write_bank(root, f, out_dir, suite_dir):
    d = json.load(open(f))
    ...
    dst = os.path.join(out_dir, os.path.basename(root))
    if suite_dir:
        dst = os.path.join(dst, os.path.basename(f).split("_qps")[0])
        print(f"  {os.path.basename(root)}: several suites harvested here; this bank is {os.path.basename(dst)}")
    ...
    json.dump(summ, open(os.path.join(dst, "summary.json"), "w"), indent=1)
