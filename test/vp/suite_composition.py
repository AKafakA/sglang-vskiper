...
def main() -> int:
    ...
    ap.add_argument("root"); ap.add_argument("--macros", required=True)
    a = ap.parse_args()
    lines, lo, hi = [], None, None
    for ds, lbl in KNEE.items():
        files = sorted(f for f in glob.glob(f"{a.root}/harvest-upstream-{ds}-{lbl}/cell/*.arrival.requests.jsonl")
                       if ".qual" not in f.rsplit("/", 1)[1])   # the serving suite, not the held-out quality suite
        if len(files) != 1:
            sys.exit(f"FATAL: expected one serving-suite arrival record for harvest-upstream-{ds}-{lbl}, found {len(files)}")
