...
stack-level delta row; plus macros for the prose. Quality = GSM8K flexible-extract accuracy, BBH exact match, CoQA token F1
(the labeled-workload scorer). Output TPS = completed x mean output tokens / makespan. A delta here combines the checkpoint's
generation behaviour with the runtime; it is NOT a runtime-effect estimate (that is the matched-work table).

usage: natural_lane_table.py summary.json --rows out_rows.tex --macros out_macros.tex
"""
...
def quality(rec, ds):
    return rec["acc_flex"] if ds == "gsm8k" else rec["acc_strict"]
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("summary"); ap.add_argument("--rows", required=True); ap.add_argument("--macros", required=True)
    ...
    for ds, labels in LABELS.items():
        for lbl, rate in labels:
            ...
            for arm, r in (("upstream", u), ("vskipper", v)):
                rows.append(f"{NAMES[ds] if arm == 'upstream' else ''} & {rate if arm == 'upstream' else ''} & {STACK[arm]} & {100*quality(r, ds):.1f} & {r['cap_hits']} & {r['mean_out_tokens']:.0f} & {r['mean_ttft_ms']:.0f} & {r['mean_tpot_ms']:.1f} & {r['mean_e2e_s']:.1f} & {tps(r):.0f} & {r['duration_s']:.0f} \\\\")
            pct = lambda k: 100 * (v[k] - u[k]) / u[k]
            dq = 100 * (quality(v, ds) - quality(u, ds)); dtps = 100 * (tps(v) - tps(u)) / tps(u)
