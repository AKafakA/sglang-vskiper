...
def load_exemptions(design_tree: Path) -> dict[str, dict[str, Any]]:
    env = dict(os.environ)
    env["PYTHONPATH"] = str(design_tree / "python")
    code = ("import json; from sglang.srt.vpipe import design; "
            "print(json.dumps(design.SERVED_LAUNCH_EXEMPTIONS, default=str))")
    done = subprocess.run([sys.executable, "-c", code], env=env, capture_output=True, text=True)
    if done.returncode != 0:
        sys.exit(f"could not read SERVED_LAUNCH_EXEMPTIONS from {design_tree}:\n"
                 f"{done.stderr[-2000:]}")
    return json.loads(done.stdout.strip().splitlines()[-1])
...
def main() -> int:
    ...
    exemptions = load_exemptions(args.design_tree)
    ...
    print(f"default conformance: {len(declared)} declared, {len(undeclared)} undeclared "
          f"(defaults from {args.defaults_tree}, exemptions from design.py)")
