...
| `../refactor_ab/hpc_preflight.sh` / `hpc_ab_arm.sh` / `hpc_ab_chain.sh` | the the HPC cluster INTR A/B chain (fail-closed preflight → per-arm serve+probe → chain driver); `hpc_ab_run.sh` + `hpc_ab_compare.py` were already in `refactor_ab/`. |
...
`VP_GATE_WORKDIR` (`/local/scratch/tmp/operator`) · `VP_GATE_TREE` (default:
...
and its `hpc_ab_run.final.sh` instance (the in-repo generalized
`refactor_ab/hpc_ab_run.sh` is NEWER — it added the stale-rep freshness
