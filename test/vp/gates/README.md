...
| `../refactor_ab/csd3_preflight.sh` / `csd3_ab_arm.sh` / `csd3_ab_chain.sh` | the the HPC cluster INTR A/B chain (fail-closed preflight → per-arm serve+probe → chain driver); `csd3_ab_run.sh` + `csd3_ab_compare.py` were already in `refactor_ab/`. |
