...
TYPES = (ROOT / "python/sglang/srt/vpipe/types.py").read_text()
SKIPPER = (ROOT / "python/sglang/srt/vpipe/skipper.py").read_text()
COMMON = (ROOT / "python/sglang/srt/vpipe/common.py").read_text()
VALIDATION = (ROOT / "python/sglang/srt/vpipe/validation.py").read_text()
EXAMPLE = (ROOT / "test/vp/example_skipper_plugin.py").read_text()
...
def test_the_routed_layer_parameter_is_no_longer_named_after_flexidepth():
    ...
    seam = (ROOT / "python/sglang/srt/vpipe/seam.py").read_text()
...
def test_the_example_is_not_wired_into_the_shipped_system():
    ...
    design = (ROOT / "python/sglang/srt/vpipe/design.py").read_text()
    assert "static_depth" not in design
    hits = [p for p in (ROOT / "python").rglob("*.py")
