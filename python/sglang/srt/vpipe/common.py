...
class FullGraphDeviceRouteTape:
    ...
    mlp_actions: Optional[torch.Tensor] = None
    ...
    def sublayer_action_masks(
        self,
        layer_id: int,
        action_batch: FullGraphActionBatch,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Write independent attention and MLP actions for one layer."""

        if self.mlp_actions is None:
            raise RuntimeError("sublayer action tape has no MLP storage")
        if self.recorded_layers >= len(self.layer_order):
            raise RuntimeError("full-graph sublayer action tape overflow")
        if action_batch.adapter_name != self.adapter_name:
            raise RuntimeError(
                "full-graph skipper adapter changed inside a captured batch: "
                f"expected {self.adapter_name}, observed {action_batch.adapter_name}"
        ...
        if layer_id != expected_layer:
            raise RuntimeError(
                "full-graph sublayer tape layer order changed: "
                f"expected {expected_layer}, observed {layer_id}"
            )
        route_index = self.recorded_layers
        attention_target = self.actions[route_index].view(-1, 1)
        mlp_target = self.mlp_actions[route_index].view(-1, 1)
        if self.branch_weights is not None:
            weight_target = self.branch_weights[route_index].view(-1, 1)
            if weight_target.shape != action_batch.branch_weights.shape:
                raise RuntimeError(
                    "full-graph branch-weight tape does not match sublayer rows"
                )
            weight_target.copy_(action_batch.branch_weights)
        action_batch.write_sublayer_storage(
            attention_out=attention_target,
            mlp_out=mlp_target,
        )
        self.recorded_layers += 1
        return attention_target, mlp_target
    ...
    def require_complete(self) -> None:
        ...
        if self.mlp_actions is not None and self.mlp_actions.shape != self.actions.shape:
            raise RuntimeError("full-graph sublayer tapes have different shapes")
        if (
...
def _accumulate_device_route_digest(
    ...
    if tape.logical_request_ids is not None:
        ...
        if tape.mlp_actions is not None:
            action_planes = torch.cat((tape.actions, tape.mlp_actions), dim=0)
            layer_ids = torch.cat((tape.layer_ids, tape.layer_ids)).to(
                dtype=torch.int64
            ).view(-1, 1)
            component_ids = torch.cat(
                (
                    torch.zeros_like(tape.layer_ids),
                    torch.ones_like(tape.layer_ids),
                )
...
def resolve_full_graph_skipper(
    ...
    adaskip_capacity_present = any(
        str(values.get(key, "")).strip() for key in _ADASKIP_CAPACITY_ENVS
    )
    profile_path = str(values.get(ADASKIP_PROFILE_ENV, "") or "").strip()
    dense_reference_mlp = adaskip_dense_reference_mlp_enabled(values)
    if name == ADASKIP_FULL_GRAPH_SKIPPER:
        if mock_config_present:
            raise ValueError(
                "AdaSkip cannot consume deterministic mock settings"
            )
        if not profile_path:
            raise ValueError(f"AdaSkip requires {ADASKIP_PROFILE_ENV}")
        profile = load_adaskip_profile(profile_path)
        if profile.online_decode_extra_mlp:
            missing = [
                key
                for key in _ADASKIP_CAPACITY_ENVS
                if not str(values.get(key, "")).strip()
            ]
            if missing:
                raise ValueError(
                    "AdaSkip online decode requires " + ", ".join(missing)
                )
            capacities: tuple[Optional[int], Optional[int]] = tuple(
                _parse_adaskip_capacity(values, key)
                for key in _ADASKIP_CAPACITY_ENVS
            )
        else:
            if adaskip_capacity_present:
                raise ValueError(
                    "AdaSkip online capacities require a profile with "
                    "online_decode_extra_mlp=true"
                )
            capacities = (None, None)
        return _adaskip_adapter(
            profile_path,
            capacities[0],
            capacities[1],
            dense_reference_mlp,
    ...
    if profile_path or adaskip_capacity_present or dense_reference_mlp:
        raise ValueError(
            "AdaSkip profile/capacity/reference settings require "
            f"{FULL_GRAPH_SKIPPER_ENV}={ADASKIP_FULL_GRAPH_SKIPPER}"
        )
