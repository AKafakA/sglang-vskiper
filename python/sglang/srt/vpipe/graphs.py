...
@dataclass(frozen=True, slots=True)
class ConditionalGraphSwitchStage:
    """One route prefix followed by one device-selected body."""

    selector: torch.Tensor
    bodies: tuple[torch.cuda.CUDAGraph, ...]
    route_prefix: Optional[torch.cuda.CUDAGraph] = None
    pre_bodies: tuple[torch.cuda.CUDAGraph, ...] = ()
    side_bodies: tuple[torch.cuda.CUDAGraph, ...] = ()
    selected_side_bodies: tuple[torch.cuda.CUDAGraph, ...] = ()
    selected_side_selector: Optional[torch.Tensor] = None
...
class ConditionalCudaGraph:
    ...
    def compose(
        ...
        *,
        ...
        """Compose already-captured segments without reading the route on host."""

        return cls.compose_stages(
            stages=(
    ...
    @classmethod
    def compose_switch_stages(
        cls,
        *,
        stages: tuple[ConditionalGraphSwitchStage, ...],
        helper: ConditionalGraphHelper,
        stream: torch.cuda.Stream,
        pool: Any,
        prefix: Optional[torch.cuda.CUDAGraph] = None,
        join: Optional[torch.cuda.CUDAGraph] = None,
        post_join_stages: tuple[ConditionalGraphSwitchStage, ...] = (),
        suffix: Optional[torch.cuda.CUDAGraph] = None,
    ) -> "ConditionalCudaGraph":
        """Compose ordered device-selected SWITCH stages."""

        cls._require_runtime()
        if helper.cuda_runtime_version < 12080:
            raise ConditionalGraphUnavailable(
                "CUDA conditional SWITCH nodes require runtime 12.8 or newer"
            )
        if not stages:
            raise ValueError("conditional switch graph requires at least one stage")
        if post_join_stages and join is None:
            raise ValueError("post-join switch stages require a join body")
        all_stages = (*stages, *post_join_stages)
        for index, stage in enumerate(all_stages):
            if (
                stage.selector.device.type != "cuda"
                or stage.selector.dtype != torch.int32
                or stage.selector.numel() != 1
            ):
                raise ValueError(
                    f"conditional switch stage {index} selector must be one "
                    "CUDA int32 value"
                )
            if not stage.bodies:
                raise ValueError(
                    f"conditional switch stage {index} requires at least one body"
                )
            if (
                stage.selected_side_bodies
                and len(stage.selected_side_bodies) != len(stage.bodies)
            ):
                raise ValueError(
                    f"conditional switch stage {index} selected side bodies "
                    "must match foreground branches"
                )
            if (
                stage.selected_side_selector is not None
                and (
                    not stage.selected_side_bodies
                    or stage.selected_side_selector.device.type != "cuda"
                    or stage.selected_side_selector.dtype != torch.int32
                    or stage.selected_side_selector.numel() != 1
                )
            ):
                raise ValueError(
                    f"conditional switch stage {index} selected side selector "
                    "must be one CUDA int32 value with selected side bodies"
                )
        for index, stage in enumerate(post_join_stages):
            if (
                stage.route_prefix is not None
                or stage.pre_bodies
                or stage.side_bodies
                or stage.selected_side_bodies
            ):
                raise ValueError(
                    f"post-join switch stage {index} must contain only a "
                    "selector and branch bodies"
                )
        ...
        try:
            ...
            for stage in stages:
                ...
                for pre_body in stage.pre_bodies:
                    pre_dependencies = (
                        [dependency] if dependency is not None else None
                    )
                    dependency = checkCudaErrors(
                        cuda_rt.cudaGraphAddChildGraphNode(
                            graph,
                            pre_dependencies,
                            len(pre_dependencies or ()),
                            cls._raw_graph(pre_body),
                        )
                    )
                ...
                foreground_setter_dependency = dependency
                if stage.selected_side_bodies:
                    side_condition = checkCudaErrors(
                        cuda_rt.cudaGraphConditionalHandleCreate(
                            graph, 0, assign_default
                        )
                    )
                    condition_handles.append(side_condition)
                    side_setter_graph = torch.cuda.CUDAGraph(keep_graph=True)
                    setter_graphs.append(side_setter_graph)
                    with torch.cuda.stream(stream):
                        side_setter_graph.capture_begin(pool=pool)
                        helper.launch_value(
                            int(side_condition),
                            (
                                stage.selected_side_selector
                                if stage.selected_side_selector is not None
                                else stage.selector
                            ),
                            stream,
                        )
                        side_setter_graph.capture_end()

                    side_setter_dependencies = (
                        [dependency] if dependency is not None else None
                    )
                    side_setter_node = checkCudaErrors(
                        cuda_rt.cudaGraphAddChildGraphNode(
                            graph,
                            side_setter_dependencies,
                            len(side_setter_dependencies or ()),
                            cls._raw_graph(side_setter_graph),
                        )
                    )
                    side_params = cuda_rt.cudaGraphNodeParams()
                    side_params.type = (
                        cuda_rt.cudaGraphNodeType.cudaGraphNodeTypeConditional
                    )
                    side_params.conditional.handle = side_condition
                    side_params.conditional.type = (
                        cuda_rt.cudaGraphConditionalNodeType.cudaGraphCondTypeSwitch
                    )
                    side_params.conditional.size = len(
                        stage.selected_side_bodies
                    )
                    side_conditional_node = checkCudaErrors(
                        cuda_rt.cudaGraphAddNode(
                            graph, [side_setter_node], None, 1, side_params
                        )
                    )
                    side_graphs = side_params.conditional.phGraph_out
                    if side_graphs is None:
                        raise ConditionalGraphUnavailable(
                            "CUDA did not return selected side SWITCH body graphs"
                        )
                    for index, body in enumerate(stage.selected_side_bodies):
                        checkCudaErrors(
                            cuda_rt.cudaGraphAddChildGraphNode(
                                side_graphs[index],
                                None,
                    ...
                    side_nodes.append(side_conditional_node)
                    # Both setters consume the selector before foreground work
                    # can advance to a scheduler prefix that mutates it.
                    foreground_setter_dependency = side_setter_node
                ...
                with torch.cuda.stream(stream):
                    setter_graph.capture_begin(pool=pool)
                    helper.launch_value(
                        int(condition), stage.selector, stream
                    )
                    setter_graph.capture_end()

                setter_dependencies = (
                    [foreground_setter_dependency]
                    if foreground_setter_dependency is not None
                    else None
                )
                setter_node = checkCudaErrors(
                    cuda_rt.cudaGraphAddChildGraphNode(
                ...
                params.conditional.size = len(stage.bodies)
                conditional_node = checkCudaErrors(
                    cuda_rt.cudaGraphAddNode(
                        graph, [setter_node], None, 1, params
                    )
                ...
                if bodies is None:
                    raise ConditionalGraphUnavailable(
                        "CUDA did not return conditional SWITCH body graphs"
                    )
                for index, body in enumerate(stage.bodies):
                    checkCudaErrors(
                        cuda_rt.cudaGraphAddChildGraphNode(
                            bodies[index], None, 0, cls._raw_graph(body)
                        )
                    )
                dependency = conditional_node
            ...
            for stage in post_join_stages:
                ...
                if bodies is None:
                    raise ConditionalGraphUnavailable(
                        "CUDA did not return post-join SWITCH body graphs"
                    )
                for index, body in enumerate(stage.bodies):
                    checkCudaErrors(
                        cuda_rt.cudaGraphAddChildGraphNode(
            ...
            return cls(
                ...
                retained_graphs=retained,
                pre_body_count=sum(
                    len(stage.pre_bodies) for stage in stages
                ),
                side_body_count=sum(
                    len(stage.side_bodies)
                    + int(bool(stage.selected_side_bodies))
                    for stage in stages
                ),
                join_body_count=int(join is not None),
                branch_count=max(len(stage.bodies) for stage in all_stages),
            )
        except Exception:
            if executable is not None:
                checkCudaErrors(cuda_rt.cudaGraphExecDestroy(executable))
...
class ConditionalGraphHelper:
    ...
    def launch_value(
        ...
        if value.device.type != "cuda":
            raise ValueError("conditional graph value must be a CUDA tensor")
        if value.dtype != torch.int32 or value.numel() != 1:
            raise ValueError("conditional graph value must be one int32 value")
        status = self.library.vp_launch_set_conditional_value(
            ctypes.c_uint64(int(handle)),
        ...
        if status != 0:
            raise RuntimeError(
                f"conditional graph value setter failed with CUDA status {status}"
            )
