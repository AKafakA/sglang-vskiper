...
MAX_STEPS=27643; SAVE_STEPS=1250   # OVERRIDES below sets SAVE_STEPS=1000 for the D-840 arms
...
# [D-840, owner 2026-09-17] penalty-tuning arms: the chain stops only on a COLLAPSE (any task below stock - 30 pp); misses are reported.
fail30 = [t for t, sv in s.items() if sv.get("accuracy") is not None and g.get(t, {}).get("accuracy") is not None and g[t]["accuracy"] - sv["accuracy"] < -0.30]
print("COLLAPSE-STOP (< stock - 30 pp): %s" % fail30 if fail30 else "no collapse")
sys.exit(10 if fail30 else 0)
PY
  local rc=$?
